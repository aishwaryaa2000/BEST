...
<nav class="navbar navbar-light bg-white sticky-top justify-content-around ">
  ...
  <ul class="navbar-nav">
        <li class="nav-item">
            <h4>AUTHORIZATION OF MEMBERS LIST</h4>
        </li>
   </ul>
...
$query="SELECT * FROM `committee_members` WHERE `cmo_auth` = 0";
...
if(mysqli_num_rows($res)>0)
{
            while($row=mysqli_fetch_assoc($res))
            {
                    ...
                    <button type='submit' class='mybtn' name='approve_submit' value='$com_id' >APPROVE
                    </div></td>";
                    ?>
                    </form>
...
{
    echo "<tr align='center'><td>-</td><td>-</td><td>-</td><td> No Members to show </td><td>-</td><td>-</td></tr>";
}
?>
...
<div class="card">
 ...
 <?php
//  WHERE unique_id='$unique_id'
$query="SELECT * FROM `committee_members` WHERE `cmo_auth` =1";
$res=mysqli_query($conn,$query);
if(mysqli_num_rows($res)>0)
{
            while($row=mysqli_fetch_assoc($res))
            {
                    ...
                    <?php
                    echo "<td><div class='d-flex justify-content-around'>
                    <button type='submit' class='mybtn' name='disapprove_submit' value='$com_id' >DISAPPROVE
                    </div></td>";
                    echo "</tr>";
                    ?>
                    </form>
